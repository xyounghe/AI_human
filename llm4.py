...
FIXED_RESPONSES = {
    ...
    "介绍涡轮喷气发动机": "涡轮喷气发动机基于“吸入-压缩-燃烧-喷出”的连续热力循环产生推力。工作时，空气经进气道吸入，在多级压气机中被高速旋转的叶片逐级压缩，成为高压气体；随后涌入燃烧室，与燃料混合剧烈燃烧，形成高温高压燃气；燃气向后冲出，驱动涡轮高速旋转，而涡轮通过同轴带动压气机持续工作，使循环得以自动维持；最后，剩余燃气在尾喷管中继续膨胀加速，以高速喷出，依据反作用力原理产生强大的向前推力。这一过程将燃料的化学能高效转化为燃气的动能。涡喷发动机完全依赖高速喷气推进，因此适合高空超音速飞行，但低速时油耗较高。",
}
import re

def split_into_sentences(text):
    """
    智能中英文分句，适合 TTS / 语音流式系统
    """
    text = text.replace("\n", "")
...
def llm_response(message, nerfreal: BaseReal):
    ...
    if message.strip() in FIXED_RESPONSES:
        ...
        for sent in sentences:
            if sent:
                logger.info(sent)
                nerfreal.put_msg_txt(sent)
                time.sleep(0.05)  # 微小节奏控制，极大降低TTS堵塞风险
