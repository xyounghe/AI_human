...
class BaseTTS:
    def __init__(self, opt, parent:BaseReal):
        ...
        self.tts_options = {
            "speaker": getattr(opt, "QWEN_SPEAKER", None),
            "language": getattr(opt, "QWEN_LANGUAGE", "Auto"),
        }
    ...
    def process_tts(self,quit_event):        
        while not quit_event.is_set():
            ...
            try:
                ...
                logger.exception('txt_to_audio error, text=%s', msg[0] if msg else '')
                self.state = State.PAUSE
    ...
    def set_tts_options(self, options: dict):
        if not isinstance(options, dict):
            return
        for key in ("speaker", "language"):
            if key in options and options[key] is not None:
                self.tts_options[key] = options[key]

    def get_tts_options(self):
        return {
            "speaker": self.tts_options.get("speaker"),
            "language": self.tts_options.get("language"),
        }
...
class QwenTTS(BaseTTS):
    def __init__(self, opt, parent):
        ...
        self.max_new_tokens = getattr(opt, "QWEN_MAX_NEW_TOKENS", 1024)
        model_path = getattr(opt, "QWEN_MODEL_PATH", "Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice")
        device = getattr(opt, "QWEN_DEVICE", "cuda:0")
        dtype_name = getattr(opt, "QWEN_DTYPE", "bfloat16").lower()
        attn_impl = getattr(opt, "QWEN_ATTN_IMPLEMENTATION", "flash_attention_2")
        ...
        self.qwen_model = self._load_qwen_model(model_path, device, dtype, attn_impl)
        self._load_supported_options()
        self._normalize_default_options()
        logger.info("QwenTTS initialized, model=%s device=%s dtype=%s", model_path, device, dtype_name)

    def _load_qwen_model(self, model_path, device, dtype, attn_impl):
        try:
            from qwen_tts import Qwen3TTSModel
        except ImportError:
            project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
            qwen_repo_path = os.path.join(project_root, "Qwen3-TTS")
            if os.path.isdir(qwen_repo_path) and qwen_repo_path not in sys.path:
    ...
    def _load_supported_options(self):
        if callable(getattr(self.qwen_model.model, "get_supported_speakers", None)):
            result = self.qwen_model.model.get_supported_speakers() or []
            self.supported_speakers = list(result)
        if callable(getattr(self.qwen_model.model, "get_supported_languages", None)):
            result = self.qwen_model.model.get_supported_languages() or []
            self.supported_languages = list(result)

    def _normalize_default_options(self):
        speaker = self.tts_options.get("speaker")
        if self.supported_speakers and speaker not in self.supported_speakers:
            self.tts_options["speaker"] = self.supported_speakers[0]

        language = self.tts_options.get("language")
        if language and language.lower() == "auto":
            self.tts_options["language"] = "Auto"
        elif self.supported_languages and language not in self.supported_languages:
            self.tts_options["language"] = "Auto"

    def set_tts_options(self, options: dict):
        super().set_tts_options(options)
        self._normalize_default_options()

    def get_tts_options(self):
        options = super().get_tts_options()
        options.update({
    ...
    def txt_to_audio(self, msg: tuple[str, dict]):
        text, textevent = msg
        speaker = textevent.get("speaker") or self.tts_options.get("speaker")
        language = textevent.get("language") or self.tts_options.get("language") or "Auto"

        if self.supported_speakers and speaker not in self.supported_speakers:
            speaker = self.supported_speakers[0]
        if self.supported_languages and language not in self.supported_languages and language != "Auto":
            language = "Auto"

        start_time = time.time()
        wavs, sample_rate = self.qwen_model.generate_custom_voice(
            text=text,
            language=language,
            speaker=speaker,
            max_new_tokens=self.max_new_tokens,
        )
        logger.info("qwentts time: %.4fs", time.time() - start_time)

        if not wavs:
            logger.error("qwentts returned empty audio")
            return

        stream = np.asarray(wavs[0], dtype=np.float32)
        if stream.ndim > 1:
            stream = np.mean(stream, axis=1).astype(np.float32)
        if sample_rate != self.sample_rate and stream.shape[0] > 0:
            stream = resampy.resample(x=stream, sr_orig=sample_rate, sr_new=self.sample_rate)

        if stream.shape[0] == 0:
            logger.error("qwentts generated zero-length audio")
            return

        total_chunks = max(1, int(np.ceil(stream.shape[0] / self.chunk)))
        for i in range(total_chunks):
            if self.state != State.RUNNING:
                break

            start_idx = i * self.chunk
            end_idx = start_idx + self.chunk
            frame = stream[start_idx:end_idx]
            if frame.shape[0] < self.chunk:
                frame = np.pad(frame, (0, self.chunk - frame.shape[0]), mode="constant")
